...
const AddGroupForm = ({
  ...
  useEffect(() => {
    if (info.userName) {
      setFormData((prev) => ({
        ...prev,
        adminName: info.userName || prev.adminName,
        email: info.userEmail || prev.email,
      }));
    }
  }, [info]);
  ...
  const submitButtonStyle =
    `bg-brand-soft-green text-brand-white text-lg mt-2 px-6 py-2 rounded-3xl transition-all cursor-pointer hover:brightness-105 disabled:opacity-50 disabled:cursor-not-allowed ` +
    focusStyle;
